...
def get_article(url):
    ...
    try:
        ...
        pass
    return {"content": article.text, "keywords": article.keywords}


def get_best_n(text, n=5):
    ...
    best_n = heapq.nlargest(n, sent2score, key=sent2score.get)
    return best_n


def get_summary(url, n=5):
    article = get_article(url=url)
    article_summary = get_best_n(article['content'], n=n)
    keywords = article['keywords']
    summary = []

    for x in article_summary:
        x = x.replace("\\", "").replace("-", " ")
        summary.append(x)
